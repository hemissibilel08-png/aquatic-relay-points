-- ====================================================
-- BIANCOTTO V2 - RLS, Politiques et Seeds
-- ====================================================
...
-- Fonction pour obtenir le staff actuel
CREATE OR REPLACE FUNCTION public.get_current_staff()
RETURNS TABLE(staff_id UUID, role staff_role, centre_id UUID)
LANGUAGE SQL
STABLE
SECURITY DEFINER
SET search_path = public
AS $$
  SELECT s.id, s.role, NULL::UUID
  FROM staff s 
...
WITH CHECK (
  public.is_facilitateur() AND
  EXISTS (SELECT 1 FROM event e WHERE e.id = event_id AND e.is_active = true) AND
  -- Vérification présence pour activités supervisées
  (
    NOT EXISTS (
    ...
    public.has_active_presence_on_station(
      (SELECT s.id FROM station s WHERE s.activity_id = attempt.activity_id)
    )
  )
);
...
DECLARE
    ...
    zone_couloirs UUID;
    zone_exterieur UUID;
    station_cecifoot UUID;
    station_parcours UUID;
    station_tech UUID;
    station_enigmes UUID;
    centre_elem UUID;
    centre_mat UUID;
    activity_cecifoot UUID;
    activity_parcours UUID;
    activity_precision UUID;
    activity_tech UUID;
BEGIN
    -- Récupération event
    ...
    SELECT id INTO zone_gymnase FROM zone WHERE name = 'Gymnase';
    SELECT id INTO zone_couloirs FROM zone WHERE name = 'Couloirs';
    SELECT id INTO zone_exterieur FROM zone WHERE name = 'Extérieur';
    ...
    (
        'Cécifoot', 
        'Football à l''aveugle en autonomie complète', 
        'autonomie', 
        'coop', 
        true,
        '{"bronze": 5, "argent": 8, "or": 12}'::jsonb,
        '{"bronze": 4, "argent": 6, "or": 8}'::jsonb
    ),
    (
        'Parcours à l''aveugle',
        'Parcours d''obstacles les yeux bandés en autonomie', 
        'autonomie', 
        'endurance', 
        true,
        '{"bronze": 60, "argent": 45, "or": 30}'::jsonb,
        '{"bronze": 70, "argent": 55, "or": 40}'::jsonb
    ),
    (
        'Lancer de précision',
        'Lancer d''objets sur cibles avec validation facilitateur', 
        'supervisee', 
        'precision', 
        false,
        '{"bronze": 3, "argent": 5, "or": 8}'::jsonb,
        '{"bronze": 2, "argent": 4, "or": 6}'::jsonb
    ),
    (
        'Station Tech',
        'Station technique avec points variables', 
        'tech', 
        'precision', 
        false,
        '{}'::jsonb,
    ...
    -- Récupération IDs activités
    SELECT id INTO activity_cecifoot FROM activity WHERE name = 'Cécifoot';
    SELECT id INTO activity_parcours FROM activity WHERE name = 'Parcours à l''aveugle';
    SELECT id INTO activity_precision FROM activity WHERE name = 'Lancer de précision';
    SELECT id INTO activity_tech FROM activity WHERE name = 'Station Tech';
    
    -- Stations (~10 stations)
    INSERT INTO public.station (zone_id, name, description, qr_code, activity_id) VALUES
    (zone_exterieur, 'Cécifoot Terrain A', 'Terrain de cécifoot principal', 'QR_CECIFOOT_A_001', activity_cecifoot),
    (zone_exterieur, 'Cécifoot Terrain B', 'Terrain de cécifoot secondaire', 'QR_CECIFOOT_B_002', activity_cecifoot),
    (zone_couloirs, 'Parcours Aveugle 1', 'Premier parcours à l''aveugle', 'QR_PARCOURS_1_003', activity_parcours),
    (zone_couloirs, 'Parcours Aveugle 2', 'Second parcours à l''aveugle', 'QR_PARCOURS_2_004', activity_parcours),
    (zone_gymnase, 'Lancer Précision Nord', 'Poste de lancer nord du gymnase', 'QR_PRECISION_N_005', activity_precision),
    (zone_gymnase, 'Lancer Précision Sud', 'Poste de lancer sud du gymnase', 'QR_PRECISION_S_006', activity_precision),
    (zone_exterieur, 'Station Tech Mobile', 'Station technique déplaçable', 'QR_TECH_MOBILE_007', activity_tech),
    ...
    -- Récupération IDs stations pour énigmes
    ...
    -- Centres (Élémentaire + Maternelle)
    ...
    -- Récupération IDs centres
    SELECT id INTO centre_elem FROM centre WHERE profile = 'elementaire';
    SELECT id INTO centre_mat FROM centre WHERE profile = 'maternelle';
    
    -- Groupes par centre (Dauphins/Tortues/Requins)
