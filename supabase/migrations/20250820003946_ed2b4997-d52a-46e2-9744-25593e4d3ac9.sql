-- Créer les données de démonstration avec les bons types d'enum
...
BEGIN
  ...
  -- Créer les activités avec les bons types d'enum
  ...
  VALUES 
    ('Tir de précision', 'supervisee', 'precision', 'Viser au centre de la cible avec des balles en mousse', 10, true, true, 
     '{"passable": 50, "bon": 30, "excellent": 15}', 
     '{"passable": 70, "bon": 50, "excellent": 30}', true),
    
    ('Lancer de distance', 'autonomie', 'lance', 'Lancer le plus loin possible avec un frisbee', 10, false, true,
     '{"passable": 10, "bon": 15, "excellent": 20}',
     '{"passable": 8, "bon": 12, "excellent": 16}', true),
     
    ('Course d''endurance', 'supervisee', 'endurance', 'Parcourir le maximum de tours en 5 minutes', 10, true, false,
     '{"passable": 300, "bon": 240, "excellent": 180}',
     '{"passable": 360, "bon": 300, "excellent": 240}', true),
     
    ('Défi coopération', 'supervisee', 'coop', 'Transporter un objet fragile en équipe sans le casser', 10, true, true,
  ...
  -- Créer les stations d'abord
  INSERT INTO station (name, description, qr_code, zone_id, activity_id, is_active)
  VALUES 
    ('Station Tir de Précision', 'Vise au centre de la cible avec les balles en mousse. Plus tu es proche du centre, plus tu gagnes de points !', 'QR_STATION_TIR_DEMO_001', v_zone_int_id, v_activity_precision_id, true),
    ('Station Lancer Frisbee', 'Lance le frisbee le plus loin possible ! Technique et force sont nécessaires.', 'QR_STATION_FRISBEE_DEMO_001', v_zone_ext_id, v_activity_lance_id, true),
    ('Station Course Marine', 'Cours autour du parcours pendant 5 minutes. Compte tes tours !', 'QR_STATION_COURSE_DEMO_001', v_zone_ext_id, v_activity_endurance_id, true),
    ('Station Coopération', 'En équipe, transportez l''œuf fragile sans le casser. Communication essentielle !', 'QR_STATION_COOP_DEMO_001', v_zone_int_id, v_activity_coop_id, true)
  RETURNING id INTO v_station1_id, v_station2_id, v_station3_id, v_station4_id;
  
  -- Créer les énigmes et les associer aux stations
  INSERT INTO riddle (question, solution, hint_text, points_base, hint_malus_elem, hint_malus_mat, is_active, station_id)
  VALUES 
    ('Quel animal marin peut changer de couleur pour se camoufler ?', 'poulpe', 'Cet animal a 8 tentacules et est très intelligent', 15, -5, -3, true, v_station1_id),
    ('Combien de cœurs a une pieuvre ?', '3', 'Elle en a plus que nous mais moins que 5', 15, -5, -3, true, v_station2_id)
