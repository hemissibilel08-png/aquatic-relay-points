...
-- RLS Policy pour empêcher insert attempt supervisée sans facilitateur
CREATE POLICY "attempt_supervised_requires_facilitator"
ON public.attempt
FOR INSERT
WITH CHECK (
  -- Si l'activité est supervisée, vérifier présence facilitateur
  NOT EXISTS (
    SELECT 1 FROM activity a
    JOIN station s ON s.activity_id = a.id
    JOIN attempt att ON att.activity_id = a.id
    WHERE a.requires_facilitator = true
      AND att.activity_id = attempt.activity_id
      AND NOT has_facilitator_on_station(
        (SELECT station_id FROM attempt WHERE id = attempt.id LIMIT 1)
      )
  )
);